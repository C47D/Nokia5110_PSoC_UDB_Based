...
/* Enable SPI */
    ...
    `$INSTANCE_NAME`_SendCommand(0x21); // `$INSTANCE_NAME`_ Extended Commands.
    `$INSTANCE_NAME`_SendCommand(0xC0); // Set `$INSTANCE_NAME`_ Vop (Contrast).
    `$INSTANCE_NAME`_SendCommand(0x06); // Set Temp coefficent.
    `$INSTANCE_NAME`_SendCommand(0x13); // `$INSTANCE_NAME`_ bias mode 1:48.
    `$INSTANCE_NAME`_SendCommand(0x20); // `$INSTANCE_NAME`_ Standard Commands, Horizontal addressing mode.
    `$INSTANCE_NAME`_SendCommand(0x0C); // `$INSTANCE_NAME`_ in normal mode.
/* Clear display */
...
void `$INSTANCE_NAME`_Chr ( char ch , font_type ft ){
    ...
    switch(ft){
    case 0: // Small font
        for(i=0; i<3; i++) `$INSTANCE_NAME`_SendData(SmallFont[ch-32][i]);// empty space after character 
        `$INSTANCE_NAME`_SendData(0x00);
    break;
...
void `$INSTANCE_NAME`_SendCommand ( char command ){
    ...
    `$INSTANCE_NAME`_SPI_WriteTxData(command);
    // Wait until SPI cycle complete.
    while ( ! (`$INSTANCE_NAME`_SPI_ReadTxStatus() & `$INSTANCE_NAME`_SPI_STS_SPI_IDLE) );
